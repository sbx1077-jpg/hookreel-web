...
(function () {
  ...
  // Barra fija: se esconde al bajar, reaparece al subir
  ...
  if (topbarEl) {
    ...
    var updateTopbar = function () {
      ...
      if (y > lastY + 4 && y > 90) {
        topbarEl.classList.add("mk-hide");
      } else if (y < lastY - 4 || y <= 90) {
        topbarEl.classList.remove("mk-hide");
      }
      lastY = y;
    };
    addEventListener("scroll", function () {
      if (!tbTicking) { tbTicking = true; requestAnimationFrame(updateTopbar); }
    }, { passive: true });
  }
  ...
  // Scroll deslizante: amortigua la rueda del ratón (reparte cada "clic" de
  // rueda en varios frames con una curva suave) para que se sienta menos a
  // saltos. No toca el motor de la película ni el arrastre del raíl, y el
  // trackpad/táctil sigue siendo 100% nativo (ya es fluido de por sí).
  if (!reduced) {
    var wheelTarget = null;
    var wheelRAF = null;
    var stepWheel = function () {
      var cur = window.scrollY;
      var diff = wheelTarget - cur;
      if (Math.abs(diff) < 0.5) {
        window.scrollTo(0, wheelTarget);
        wheelRAF = null; wheelTarget = null;
        return;
      }
      window.scrollTo(0, cur + diff * 0.25);
      wheelRAF = requestAnimationFrame(stepWheel);
    };
    addEventListener("wheel", function (e) {
      if (e.ctrlKey) return; // no interferir con el zoom
      if (Math.abs(e.deltaY) < 1) return;
      if (rail && rail.classList.contains("mk-dragging")) return;
      e.preventDefault();
      var max = document.documentElement.scrollHeight - innerHeight;
      var base = wheelTarget === null ? window.scrollY : wheelTarget;
      wheelTarget = Math.max(0, Math.min(max, base + e.deltaY));
